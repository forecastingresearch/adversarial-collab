...
rankings <- reshapedData %>%
  group_by(ID) %>%
  mutate(rank = rank(VoI_log, na.last = "keep", ties.method = "min")) %>%
  group_by(ID) %>%
  summarise(mean_rank = mean(rank, na.rm = TRUE),
            mean_rank_skeptic = mean(rank[Camp == "Skeptical"], na.rm = TRUE),
            mean_rank_concerned = mean(rank[Camp == "Concerned"], na.rm = TRUE)) %>%
  arrange(mean_rank)

write.csv(rankings, "mean_ranks_KL.csv", row.names = FALSE)
```

```{r}
# NAIVE VOI: First, rank questions by person. Then take average rank of each question
rankings <- reshapedData %>%
  group_by(ID) %>%
  mutate(rank = rank(VoI_naive, na.last = "keep", ties.method = "min")) %>%
  group_by(ID) %>%
  summarise(mean_rank = mean(rank, na.rm = TRUE),
            mean_rank_skeptic = mean(rank[Camp == "Skeptical"], na.rm = TRUE),
            mean_rank_concerned = mean(rank[Camp == "Concerned"], na.rm = TRUE)) %>%
...
write.csv(rankings, "mean_ranks_naive.csv", row.names = FALSE)
```

```{r}
# Plot overlapping histograms of VoI_log for each question
argh <- reshapedData

...
bresults <- reshapedData %>%
  group_by(Camp, ID) %>%
  summarize(median = quantile50(VoI_log),
...
jazz <- reshapedData %>%
  ggplot(aes(x = factor(ID, levels = order), y = Pc, color = Camp)) +
  ...
  labs(x = "Question", y = "P(Question Resolves \"Yes\")", color = "Camp") +
  theme_minimal() +
  scale_color_manual(values = palette2) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("pc_by_question.png", jazz, width = 15, height = 8)
...
lookup <- reshapedData %>%
  group_by(ID, Camp) %>%
  summarize(happens = quantile50(PUc),
...
juice <- reshapedData %>%
  group_by(ID, Camp) %>%
  summarize(medianVOI = quantile50(VoI_log),
...
juice_naive <- reshapedData %>%
  group_by(ID, Camp) %>%
  summarize(medianVOI = quantile50(VoI_naive),
            medianPercentMaxVOI = quantile50(VoI_naive / VoI_naive(PU, 1 - (1E-16), PU))) %>%  # Divide by max naive VOI
  pivot_wider(names_from = Camp, values_from = c(medianVOI, medianPercentMaxVOI)) %>%
  select(ID, medianVOI_Concerned, medianPercentMaxVOI_Concerned, medianVOI_Skeptical, medianPercentMaxVOI_Skeptical)
...
compare_medians <- function(id1, id2, data) {
  medians_id1 <- data %>% filter(ID == id1) %>% pull(medians)
  medians_id2 <- data %>% filter(ID == id2) %>% pull(medians)
...
comparison_matrix <- matrix(0, nrow = matrix_size, ncol=matrix_size,
...
# Function to compare medians between two IDs and count occurrences
compare_medians <- function(id1, id2, data) {
  medians_id1 <- data %>% filter(ID == id1) %>% pull(medians_VoD)
  medians_id2 <- data %>% filter(ID == id2) %>% pull(medians_VoD)
  sum(medians_id1 > medians_id2)
}
...
matrix_size <- length(unique_IDs)
comparison_matrix_VoD <- matrix(0, nrow=matrix_size, ncol=matrix_size,
                                dimnames=list(unique_IDs, unique_IDs))
...
heatmap_plot <- ggplot(comparison_df, aes(x = Var1, y = Var2, fill = Freq)) +
  ...
  theme_minimal() +
  labs(title = paste0("Comparison Heatmap for ", currCamp, " (R=", R, ")"),
       x = "ID 1",
       y = "ID 2",
...
heatmap_plot <- ggplot(comparison_df_VoD, aes(x = Var1, y = Var2, fill = Freq)) +
  ...
  scale_fill_gradientn(colours = topo.colors(10)) + # You can choose your own color gradient
  theme_minimal() +
  labs(title = paste0("Comparison Heatmap for VoD (R=", R, ")"),
       x = "ID 1",
       y = "ID 2",
...
results_df_cross_camp <- results_df_cross_camp %>%
  mutate(ID = gsub("_.+$", "", ID_pair_statistic),
...
results_df_cross_camp_agg <- results_df_cross_camp %>%
  group_by(ID) %>%
  summarize(median_VoD = median(VoD, na.rm = TRUE),
...
VoD_quantiles <- results_df_cross_camp %>%
  group_by(ID) %>%
  summarize(median = median(VoD, na.rm = TRUE),
...
bars <- ggplot(VoD_quantiles, aes(x = ID, y = median, fill = median)) +
  ...
  scale_fill_gradient2(low = "orange", high = "green", midpoint = 0,
...
best_of %>%
  group_by(Camp) %>%
  summarize(median = quantile50(percent_max_VoI),
...
for (i in 1:(numpeople-1)) {
    for (j in (i+1):numpeople) {
        a <- aa %>% filter(ID1 == people[i], ID2 == people[j]) %>% pull(value)
        b <- aa %>% filter(ID2 == people[i], ID1 == people[j]) %>% pull(value)
        if (length(a) == 0) {
            vod_matrix[i, j] <- b[1]
...
```{r}
reshapedData %>% filter(grepl("James", person) | grepl("Xander", person), grepl("Evidence", ID)) %>% select(Camp, PU, Pc, PUc, ID, person, punotc, percent_max_VoI)
```

```{r}
# How many times does each question come up in aa$value
aa %>% group_by(C) %>% summarize(n = n()) %>% arrange(desc(n)) %>% print(n = 31)
```

```{r}
stacked <- reshapedData %>%
  group_by(ID, Camp) %>%
  summarize(percent_up = sum(direction == "up") / n() * 100,
...
reshapedData %>% group_by(ID, Camp, direction) %>% summarize(n = n())
```

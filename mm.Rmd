...
# For each question (ID) in reshapedData, for each pair of people (person), calculate the VoD_log_geomean
...
for (id in unique(reshapedData$ID)) {
  ...
  for (col in seq(1, ncol(combinations))) {
    ...
    statistic_value <- VoD_log_geomean(dt_subset, id1, id2)
...
# Now give me variance of VoD_log_geomean for each question
...
# VoD ~ 0
xander_v_vincent <- VoD_log_geomean(gdp, "Xander (Concerned)", "Vincent (Concerned)")
# VoD ~ 4
xander_v_quentin <- VoD_log_geomean(gdp, "Xander (Concerned)", "Quentin (Concerned)")
# Very negative VoD
ume_v_hank <- VoD_log_geomean(gdp, "Ume (Concerned)", "Hank (Skeptical)")
ume_v_ash <- VoD_log_geomean(gdp, "Ume (Concerned)", "Ash (Skeptical)")
ume_v_james <- VoD_log_geomean(gdp, "Ume (Concerned)", "James (Skeptical)")
ume_v_claire <- VoD_log_geomean(gdp, "Ume (Concerned)", "Claire (Skeptical)")
```

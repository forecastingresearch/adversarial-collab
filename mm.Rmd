...
puc_s = 1e-15
pc_s = pu_s
VoI_log_s = VoI_log(pu_c, puc_c, pc_c)
...
f(p, i, c) = p * c + ((i - p * c)/(1-c)*log((i - p * c)/(1-c)/0.001) + (1-(i - 0.0172334 * c)/(1-c))*log((1-(i - p * c)/(1-c))/(1-0.001)))*(1-c)

i might have made a mistake there

p is P(U|c), c is P(c), i is P(U) (that's the starting point)

If i is .235, can we find p and c that maximize f(p, i, c)?
...
```{r}
exQuestion <- reshapedData %>% filter(grepl("AI coding", ID))

VoD_log_gmod_easy(exQuestion, "Yael (Concerned)", "Ash (Skeptical)")
```
...
central_tendency <- reshapedData %>%
  group_by(ID) %>%
  summarise(q50_KL = quantile(VoI_log, p = 0.5, type = 3, na.rm = TRUE),
...
results_df_cross_camp_f <- results_df_cross_camp %>%
  select(ID, value) %>%
  group_by(ID) %>%
  summarize(median_cross_camp_pairs = median(value, na.rm = TRUE))

median_forecasters_f <- median_forecasters %>%
  select(ID, VoD_log_gmod)

...
library(dplyr)
library(data.table)
gs4_auth() # run this to authenticate with google sheet (must do this before running code)
```

```{r}
spreadsheet_url <- "***REMOVED***"

# Read the 'RawForR' sheet from the Google Spreadsheet
pSheet_final <- read_sheet(spreadsheet_url, sheet = "RawForR")

# Read the Tags (short names corresponding to question IDs)
tags <- read_sheet(spreadsheet_url, sheet = "Composite") %>%
  select(Id, Tag) %>%
  rename(ID = Id)

# Merge the tags with the pSheet_final data frame
pSheet_final <- merge(pSheet_final, tags, by = "ID", all.x = TRUE) %>%
  select(-ID) %>%
  rename(ID = Tag)
```
...
# Aliases
aliases_supers <- read_sheet("***REMOVED***", sheet = "Sheet1")
aliases_experts <- read_sheet("***REMOVED***", sheet = "Sheet2")
newnames <- rbind(aliases_supers, aliases_experts)
...
```{r}
# Assuming reshapedData, palette2, and other required libraries are already loaded
...
jazz <- reshapedData %>%
  ggplot(aes(x = Pc, y = factor(ID, levels = order), color = Camp)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA, orientation = "y") + # Adjust orientation
  geom_jitter(position = position_jitterdodge(jitter.width = 0.5, dodge.width = 0.9), alpha = 0.5) +
  labs(x = "P(Question Resolves \"Yes\")", y = "Question", color = "Camp") +
  theme_minimal() +
  scale_color_manual(values = palette2) +
  theme(
    axis.text.y = element_text(hjust = 1), # Adjust axis text orientation
...
ggsave("pc_by_question.png", jazz, width = 8, height = 12)
...
# Calculate median for each camp and ID
medians <- reshapedData %>%
  group_by(ID, Camp) %>%
  summarize(median_Pc = quantile50(Pc), .groups = "drop")

# Prepare shading data and calculate differences
shade_data <- medians %>%
  spread(key = Camp, value = median_Pc) %>%
  rename(median_Concerned = `Concerned`, median_Skeptical = `Skeptical`) %>%
  mutate(difference = abs(median_Concerned - median_Skeptical))
...
# Update ID ordering in medians data frame
medians$ID <- factor(medians$ID, levels = ordered_IDs)

# Update reshapedData to have the same order
reshapedData$ID <- factor(reshapedData$ID, levels = ordered_IDs)

# Plot with jittered dots (only vertically), shaded area, and median markers
ggplot() +
  geom_jitter(data = reshapedData, aes(y = ID, x = Pc, color = Camp), width = 0, height = 0.2, alpha = 0.5, size = 3) +
  geom_rect(data = shade_data, aes(ymin = as.numeric(ID) - 0.4, ymax = as.numeric(ID) + 0.4, xmin = median_Concerned, xmax = median_Skeptical), fill = "grey", alpha = 0.5, inherit.aes = FALSE) +
  geom_point(data = medians, aes(x = median_Pc, y = as.numeric(ID), color = Camp), size = 6, shape = 124) +
  labs(y = "Question (Ordered by Difference in Medians)", x = "P(Question Resolves \"Yes\")", color = "Camp") +
  scale_color_manual(values = palette2) +
  theme_minimal() +
  theme(
...
# Save to pc_by_question.png
ggsave("pc_by_question.png", width = 10, height = 12)
```

```{r}
order <- reshapedData %>%
  group_by(ID) %>%
  summarize(median = quantile50(PUc)) %>%
  arrange(median) %>%
  pull(ID)

jazz <- reshapedData %>%
  ggplot(aes(x = factor(ID, levels = order), y = PUc, color = Camp)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) + # outlier.shape = NA to remove default outliers
  geom_jitter(position = position_jitterdodge(jitter.width = 0.5, dodge.width = 0.9), alpha = 0.5) +
  labs(x = "Question", y = "P(AI Extinction by 2100 | Question Resolves \"Yes\")", color = "Camp") +
  theme_minimal() +
  scale_color_manual(values = palette2) +
  theme(
...
ggsave("puc_by_question.png", jazz, width = 15, height = 8)
```
...
# OK now let's do that for each of mixed, skeptical and concerned, and for both Pearson and Spearman
corrs_list <- list(
  mixed_corrs = mixed_corrs,
  skeptical_corrs = skeptical_corrs,
  concerned_corrs = concerned_corrs
)

# List of correlation types
corr_types <- c("spearman_corr", "raw_corr")

# Initialize an empty list to store results
numeric_results <- list()

# Outer loop over data frames
for (corr_name in names(corrs_list)) {
  df <- corrs_list[[corr_name]]

  numpairs <- length(df$pair)
  boot_indices <- lapply(1:R, function(i) sample(1:numpairs, numpairs, replace = TRUE))

  # Inner loop over correlation types
  for (corr_type in corr_types) {
    # Perform bootstrap CI calculation
    boot_results <- df %>%
      ungroup() %>%
      summarize(
        actual_median_corr = quantile50(!!sym(corr_type)),
        boot_results = list(
          map(boot_indices, ~ boot_results_modified(df, .x, "quantile50", corr_type))
        )
      ) %>%
      unnest(boot_results)

    # Extract numeric results and store in the list
    numeric_results[[paste(corr_name, corr_type, sep = "_")]] <- sapply(boot_results[["boot_results"]], `[[`, 1)
  }
}
```
...
write.csv(juice, "juice.csv", row.names = FALSE)
```
...
write.csv(juice_naive, "juice_naive.csv", row.names = FALSE)
```
...
possum <- ggplot(bresults, aes(x = ID, y = median, group = Camp)) +
  ...
  labs(
    title = "Median VOI with Bootstrapped CIs",
    y = "Log VOI",
    x = "Question"
  ) +
...
make_fiedler_ordered_heatmap_2 <- function(mat, title) {
  ...
  # Save the plot
  ggsave(filename = paste0("p8_heatmaps/lower_triangle_", title, ".png"), plot = p, width = 8, height = 8)
}
```
...
results_df_cross_camp_agg <- results_df_cross_camp %>%
  group_by(ID) %>%
  summarize(
    median_VoD = median(VoD, na.rm = TRUE),
    median_VoD_pom = median(percent_max_VoD, na.rm = TRUE)
  ) %>%
...
```{r}
# VOD: For each question, tell me average percent of the time that the median VoD was higher than any other question

fortableD <- comparison_df_VoD %>%
  group_by(Var1) %>%
  summarize(stat = mean(Freq, na.rm = TRUE)) %>%
  rename(ID = Var1)

results_df_cross_camp_q50 <- results_df_cross_camp %>%
  group_by(ID) %>%
  summarize(value = quantile50(value))

table1 <- merge(fortableD, results_df_cross_camp_q50, by = "ID") %>%
  select(ID, value, stat) %>%
  arrange(value, ascending = FALSE)

write.csv(table1, "table_VoD.csv", row.names = FALSE)
```

```{r}
# VOI: Ditto

fortableI <- comparison_df %>%
  group_by(Var1) %>%
  summarize(stat = mean(Freq, na.rm = TRUE)) %>%
  rename(ID = Var1)

medianVOI <- central_tendency %>%
  select(ID, q50_KL_skeptic) %>% # CHANGE!!!!!
  rename(medians = q50_KL_skeptic)

table2 <- merge(fortableI, central_tendency) %>%
  select(ID, q50_KL_skeptic, stat) %>% # You have to change this!!
  arrange(q50_KL_skeptic, ascending = FALSE)

# Which camp did you just do? See currCamp
write.csv(table2, paste0("table_VOI_", currCamp, ".csv"), row.names = FALSE)
```
...
g2 <- ggplot(data = prepost) +
  ...
  labs(
    title = "Initial and Final P(doom) Forecasts",
    x = "Forecast Type",
    y = "Values"
  ) +
...
```{r}
aa %>% filter("Xander (Concerned)" == ID2, "James (Skeptical)" == ID1)
```
...
write.csv(bestVOD, "BB.csv", row.names = FALSE)
```
...
```{r}
# Extracting the order based on percent_down for Concerned group
order_index <- stacked %>%
  filter(Camp == "Concerned") %>%
  arrange(-percent_down) %>%
  pull(ID)

# Setting the order in the stacked dataframe
stacked$ID <- factor(stacked$ID, levels = order_index)

# Reshaping the data with pivot_longer
stacked_melted <- pivot_longer(stacked, cols = starts_with("percent_"), names_to = "Category", values_to = "Value")

# Rename the Categories
stacked_melted$Category[stacked_melted$Category == "percent_down"] <- "Down"
stacked_melted$Category[stacked_melted$Category == "percent_up"] <- "Up"
stacked_melted$Category[stacked_melted$Category == "percent_no_change"] <- "No Change"

# Adjusting the order of categories for stacked bars AFTER renaming
stacked_melted$Category <- factor(stacked_melted$Category, levels = c("Up", "No Change", "Down"))

# Plotting
stacks <- ggplot(stacked_melted, aes(x = ID, y = Value, fill = Category)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_grid(Camp ~ ., scales = "free_x", space = "free") +
  labs(y = "", x = "Question", fill = "Direction of\nUpdate") +
  scale_fill_manual(values = c("Up" = "#FED766", "Down" = "#56A96A", "No Change" = "#272727")) +
  theme_minimal() +
  theme(
...
ggsave("stacks_direction.png", stacks, width = 8, height = 10, dpi = 300)
```

```{r}
# Debugging stacked bar chart....
reshapedData %>%
  group_by(ID, Camp, direction) %>%
  summarize(n = n())
```
...
# Calculate "% down" for each ID within the "Concerned" camp and arrange the levels of ID based on this.
reshapedData <- reshapedData %>%
  group_by(ID) %>%
  summarize(percent_down = sum(direction == "down" & Camp == "Concerned") / n()) %>%
  arrange(-percent_down) %>%
  left_join(reshapedData) %>%
  mutate(ID = factor(ID, levels = unique(ID)))

# Plot
stacked2 <- ggplot(reshapedData, aes(x = ID, fill = direction)) +
  geom_bar(position = "fill", stat = "count", show.legend = TRUE) +
  scale_y_continuous(labels = scales::percent_format()) +
  facet_grid(Camp ~ ., scales = "free_x", space = "free") +
  labs(y = "Percent of Participants", y = "Question", fill = "Direction of\nUpdate") +
  scale_fill_manual(values = c("up" = "#FED766", "down" = "#56A96A", "no change" = "#272727")) +
  theme_minimal() +
  theme(
    axis.text.x.top = element_blank(), axis.ticks.x.top = element_blank(),
...
ggsave("stacks_direction.png", stacked2, width = 10, height = 14)
```

...
jazz <- reshapedData %>%
  ...
  labs(x = "Question", y = "P(c)", color = "Camp") +
  theme_minimal() +
...
jazz <- reshapedData %>%
  ...
  geom_boxplot(alpha = 0.7, outlier.shape = NA) + # outlier.shape = NA to remove default outliers
  geom_jitter(position = position_jitterdodge(jitter.width = 0.5, dodge.width = 0.9), alpha = 0.5) +
  labs(x = "Question", y = "P(U|c)", color = "Camp") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
plotMe <- function(reshapedData) {
  ggplot(reshapedData, aes(x = ID, y = VoI_log, color = Camp)) +
    ...
    scale_y_continuous(trans = log10_trans(),
                       breaks = trans_breaks("log10", function(x) 10^x),
                       labels = trans_format("log10", math_format(10^.x))) +
    geom_point(data = central_tendency, aes(x = ID, y = q50_KL_concerned), color = "#f8a06d", shape = 16, alpha = .4, size = 5, show.legend = FALSE) +
    geom_point(data = central_tendency, aes(x = ID, y = q50_KL_skeptic), color = "#0072c4", shape = 16, alpha = .4, size = 5, show.legend = FALSE) +

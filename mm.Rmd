...
df_log <- reshapedData %>%
  group_by(person) %>%
  mutate(rank = rank(-VoI_log, ties.method = "min", na.last = "keep"))

# Take average rank per question
avg_ranks_log <- df_log %>%
  group_by(ID) %>%
  summarize(avg_rank = mean(rank))

write.csv(avg_ranks_log, "mean_ranks_KL.csv", row.names = FALSE)
```
...
# FOR NAIVE VOI
# First, rank questions by person. Then take average rank of each question.
df_naive <- reshapedData %>%
  group_by(person) %>%
  mutate(rank = rank(-VoI_naive, ties.method = "min", na.last = "keep"))

# Take average rank per question
avg_ranks_naive <- df_naive %>%
  group_by(ID) %>%
  summarize(avg_rank = mean(rank))

write.csv(avg_ranks_naive, "mean_ranks_naive.csv", row.names = FALSE)
```
...
reshapedData <- reshapedData %>%
  ...
  mutate(numDirection = n()) %>%
  mutate(***REMOVED***Direction = ifelse(numDirection > 0.67 * n(), direction, "no ***REMOVED***")) %>%
  mutate(median_KL = median(VoI_log, na.rm = TRUE))
```

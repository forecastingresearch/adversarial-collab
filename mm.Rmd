...
# Update shade_data for geom_rect
shade_data <- shade_data %>%
  mutate(ID = as.numeric(factor(ID, levels = ordered_IDs)))

# Plot with jittered dots and shaded area, with larger text
ggplot(reshapedData, aes(y = ID, x = Pc, color = Camp)) +
  geom_jitter(width = 0.2, alpha = 0.5, size = 3) +
  geom_rect(data = shade_data, aes(ymin = ID - 0.4, ymax = ID + 0.4, xmin = median_Concerned, xmax = median_Skeptical), fill = "grey", alpha = 0.5, inherit.aes = FALSE) +

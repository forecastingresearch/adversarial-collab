...
```{r}
# Holden/Benjamin's alt VOI

# Same for bresults
bresults_ranks <- bresults %>%
  group_by(Camp) %>%
  mutate(rank_original = rank(-median, ties.method = "min")) %>%
  select(-confint_lower, -confint_upper)

cherry3 <- reshapedData %>%
  select(-VoI_log, -VoI_naive)

# We want the opposite camp's PC, so let's swap the camps in PC and make it long
PC_cherry3 <- PC %>%
  pivot_longer(cols = c(Concerned, Skeptical), names_to = "Camp", values_to = "PC") %>%
  mutate(Camp = ifelse(Camp == "Concerned", "Skeptical", "Concerned")) %>%
  rename(PC_alt = PC)

# Merge the PC data with the reshapedData
cherry3 <- merge(cherry3, PC_cherry3, by = c("ID", "Camp")) %>%
  rowwise() %>%
  mutate(
    VoI_log = VoI_log(PU, PUc, pc = Pc, punotc = punotc, pc_weight = PC_alt)
  )

write.csv(cherry3, "counterpart-voi.csv", row.names = FALSE)
```
...
# Add ranks for both camps
cherry_agg_ranks3 <- cherry_agg3 %>%
  group_by(Camp) %>%
  mutate(rank_alt = rank(-median, ties.method = "min"))

# Merge the two data frames
cherry_agg_ranks3 <- merge(cherry_agg_ranks3, bresults_ranks, by = c("ID", "Camp")) %>%
  arrange(rank_alt)

write.csv(cherry_agg_ranks3, "counterpart-voi-agg.csv", row.names = FALSE)
```

```{r}
# P(c) by question
p_by_question_graph(reshapedData, "Camp", "P(Question Resolves \"Yes\")", "Pc", "ID", palette2, "figures/pc_by_question.png")

# P(U|c) by question
p_by_question_graph(reshapedData, "Camp", "P(AI Extinction by 2100 | Question Resolves \"Yes\")", "PUc", "ID", palette2, "figures/puc_by_question.png")
```
...
ggsave("corr_graphs/pearson_corrs.png", width = 9, height = 8)
```
...
ggsave("corr_graphs/spearman_corrs.png", width = 9, height = 8)
```
...
juice_naive <- reshapedData %>%
  rowwise() %>%
  mutate(maxVOI = VoI_naive(PU, 1 - (1E-16), PU)) %>%
  group_by(ID, Camp) %>%
  summarize(
    medianVOI = quantile50(VoI_naive),
    medianPercentMaxVOI = quantile50(VoI_naive / maxVOI)
...
plotMe <- function(reshapedData) {
  ggplot(reshapedData, aes(x = ID, y = VoI_log, color = Camp)) +
    ...
    scale_y_continuous(
      trans = log10_trans(),
      breaks = 10^seq(-34, -1, by = 3), # Breaks at 10^-14, 10^-12, ..., 10^-2
...
plotMe(reshapedData)
ggsave("figures/VoI_log.png", width = 10, height = 8)
...
plotMe(reshapedData)
ggsave("figures/VoI_log_ordered_concerned.png", width = 10, height = 7)

reshapedData$ID <- factor(reshapedData$ID, levels = central_tendency$ID[order(central_tendency$q50_KL_skeptic)])

plotMe(reshapedData)
ggsave("figures/VoI_log_ordered_skeptic.png", width = 10, height = 7)
```
...
make_fiedler_ordered_heatmap_2 <- function(mat, title) {
  ...
  # Save the plot
  ggsave(filename = paste0("heatmaps/lower_triangle_", title, ".png"), plot = p, width = 8, height = 7)
}
```
...
results_df_cross_camp_f <- results_df_cross_camp %>%
  select(ID, VoD) %>%
  group_by(ID) %>%
  summarize(median_cross_camp_pairs = quantile50(VoD))
  ...
  mutate(
    majorityDirection =
      ifelse(numUp > 0.67 * total, "up",
        ifelse(numDown > 0.67 * total, "down", "no majority")
      )
  )
...
write.csv(directions2, "directions.csv", row.names = FALSE)
```
